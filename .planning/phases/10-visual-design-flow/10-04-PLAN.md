...
must_haves:
  ...
  artifacts:
    ...
    - path: ".planning/ROADMAP.md"
      provides: "Corrected FLOW-01 requirement"
      contains: "Welcome -> Auth -> CreateEventType -> LogFirstEvent -> Permissions"
  key_links:
...
NOTE: This plan also updates ROADMAP.md to correct FLOW-01. The original FLOW-01 stated "Welcome -> Auth -> Permissions" but the actual implemented flow (from prior milestone work) is "Welcome -> Auth -> CreateEventType -> LogFirstEvent -> Permissions -> Finish". The roadmap requirement was documented incorrectly and needs to match the intentional flow.
</objective>
...
<task type="auto">
  <name>Task 1: Update OnboardingStep Progress Calculation and Fix ROADMAP FLOW-01</name>
  <files>
    apps/ios/trendy/Models/Onboarding/OnboardingStep.swift
    .planning/ROADMAP.md
  </files>
  <action>
**Update OnboardingStep.swift:**
...
Find the FLOW-01 requirement under Phase 10 and update it from:
```
...
This reflects the actual intentional flow from prior milestone work. The CreateEventType and LogFirstEvent steps were added to get users engaged with the app during onboarding, before asking for permissions.
  </action>
  <verify>
1. OnboardingStep.swift compiles without errors
2. OnboardingStep.welcome.progress == 0.0
3. OnboardingStep.finish.progress == 1.0
4. ROADMAP.md contains "Welcome -> Auth -> CreateEventType -> LogFirstEvent -> Permissions -> Finish"
  </verify>
  <done>OnboardingStep has progress computed property, ROADMAP.md FLOW-01 corrected to match actual flow</done>
</task>
...
LogFirstEventView currently has `ProgressIndicatorView(currentStep: 3, totalSteps: 4)` at line 71 inside the logEventView computed property.

Make these changes:

1. Add OnboardingProgressBar at the TOP of the main VStack (line 22, after `VStack(spacing: 24) {`), BEFORE the first Spacer:
   ```swift
...
2. Remove the old `ProgressIndicatorView(currentStep: 3, totalSteps: 4)` from line 71 in logEventView (it's between the header VStack and the Event Type Display).

3. Keep all existing functionality:
   - The logEventView and successView structure
   - The eventType display with icon
   - The notes TextField
   - The "Log Now" button
   - The "Skip for now" button at bottom
   - The success animation and auto-advance
...
4. LogFirstEventView has OnboardingProgressBar at top and no ProgressIndicatorView
5. LaunchLoadingView has updated aesthetic
  </verify>
...
8. ROADMAP.md FLOW-01 updated to reflect actual flow
</verification>
...
8. ROADMAP.md FLOW-01 matches actual implementation
</success_criteria>
