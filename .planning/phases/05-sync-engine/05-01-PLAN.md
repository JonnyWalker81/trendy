...
4. In `setModelContext()`, call refreshSyncStateForUI() ONLY if syncEngine is non-nil:
```swift
// At end of setModelContext(), after setting up syncEngine:
if syncEngine != nil {
    Task {
...
IMPORTANT: The guard in refreshSyncStateForUI() handles nil syncEngine, but in setModelContext() we should avoid spawning unnecessary Tasks when syncEngine is nil (e.g., when not authenticated).
  </action>
