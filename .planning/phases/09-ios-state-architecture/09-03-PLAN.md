...
must_haves:
  truths:
    ...
    - "LoginView calls AppRouter.handleLogin() after successful authentication"
  artifacts:
    ...
    - path: "apps/ios/trendy/Views/Auth/LoginView.swift"
      provides: "Login view that wires to AppRouter on success"
      contains: "appRouter.handleLogin"
  key_links:
    ...
    - from: "apps/ios/trendy/Views/Auth/LoginView.swift"
      to: "apps/ios/trendy/Services/AppRouter.swift"
      via: "onChange of authViewModel.isAuthenticated"
      pattern: "onChange.*isAuthenticated.*handleLogin"
---
...
Output: OnboardingViewModel and OnboardingContainerView use AppRouter, ContentView simplified, LoginView wired to AppRouter.
</objective>
...
<task type="auto">
  <name>Task 3: Simplify ContentView (remove routing logic)</name>
  <files>
    apps/ios/trendy/ContentView.swift
  </files>
  <action>
...
2. Grep for any remaining onboardingCompleted notifications in ContentView: `grep -n "onboardingCompleted\|NotificationCenter" apps/ios/trendy/ContentView.swift` should return empty (or only the extension definition)
  ...
  <done>
ContentView simplified to screenshot-mode only, no routing logic.
  </done>
</task>

<task type="auto">
  <name>Task 4: Wire LoginView to AppRouter on auth success</name>
  <files>
    apps/ios/trendy/Views/Auth/LoginView.swift
  </files>
  <action>
...
LoginView needs to call AppRouter after successful login. The challenge is that `AuthViewModel.signIn()` doesn't return success/failure - it sets `authViewModel.isAuthenticated`.

**Solution: Use onChange observer on isAuthenticated**
...
2. Add a flag to track if we initiated a login attempt:
```swift
@State private var isLoggingIn = false
```

3. Before the login button action, set the flag:
```swift
Button {
    isLoggingIn = true  // Track that we initiated login
    Task {
...
4. Add onChange observer to detect successful login:
```swift
...
This pattern:
- Only triggers when isAuthenticated changes from false to true
- Only triggers if we were in the middle of a login attempt
- Calls appRouter.handleLogin() which syncs onboarding status and routes appropriately

5. Reset the flag on error (in case user tries again):
Add to the error display section or create an onChange for errorMessage:
```swift
...
**Alternative simpler approach if preferred:**
Modify AuthViewModel to return Bool from signIn(), but this requires changes to AuthViewModel which is out of scope for this plan. The onChange approach works with existing AuthViewModel API.
  </action>
  <verify>
1. Build succeeds: `cd apps/ios && xcodebuild -scheme "Trendy (Local)" -destination "platform=iOS Simulator,name=iPhone 16" build 2>&1 | tail -20`
2. Grep for appRouter.handleLogin in LoginView: `grep -n "appRouter.handleLogin\|handleLogin" apps/ios/trendy/Views/Auth/LoginView.swift` should show the call
  </verify>
  <done>
...
- [ ] LoginView has @Environment(AppRouter.self)
- [ ] LoginView calls appRouter.handleLogin() via onChange of isAuthenticated
