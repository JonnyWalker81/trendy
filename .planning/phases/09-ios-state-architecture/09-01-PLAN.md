...
must_haves:
  truths:
    ...
    - "Returning user with cached status sees correct screen instantly (no loading flash)"
...
- Added `createEventTypeCompletedAt` and `logFirstEventCompletedAt` for local tracking of steps not in backend
...
- `markStepCompleted()` handles ALL 6 OnboardingStep cases explicitly (no default)
- `.createEventType` and `.logFirstEvent` are tracked in cache only (backend doesn't have these columns)
- Backend sync only happens for steps the backend tracks (.welcome, .auth, .permissions, .finish)
...
Build succeeds: `cd apps/ios && xcodebuild -scheme "Trendy (Local)" -destination "platform=iOS Simulator,name=iPhone 16" build 2>&1 | tail -20`
  ...
  <done>
OnboardingStatusService provides synchronous cache reads, backend sync with timeout, and step completion tracking with explicit handling for all 6 OnboardingStep cases.
  </done>
</task>
...
5. markStepCompleted() switch has explicit cases for all 6 OnboardingStep values (no default)
</verification>
...
- [ ] markStepCompleted() handles .createEventType and .logFirstEvent explicitly (not via default)
