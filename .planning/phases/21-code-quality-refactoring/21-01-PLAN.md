...
```bash
cd /Users/cipher/Repositories/trendy && \
  FILE="apps/ios/trendy/Services/Sync/SyncEngine.swift" && \
  START=$(grep -n 'private func flushPendingMutations' "$FILE" | head -1 | cut -d: -f1) && \
  END=$(awk -v s="$START" 'NR > s && /^    private func / {print NR; exit}' "$FILE") && \
  sed -n "${START},$((END - 1))p" "$FILE" | wc -l
```
