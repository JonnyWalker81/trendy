...
func (s *syncService) GetSyncStatus(ctx context.Context, userID string) (*SyncStatus, error) {
	...
	wg.Add(7)
	go func() { defer wg.Done(); c, e := s.eventRepo.CountByUser(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); eventCount = c; mu.Unlock() }()
	go func() { defer wg.Done(); c, e := s.eventTypeRepo.CountByUser(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); eventTypeCount = c; mu.Unlock() }()
	go func() { defer wg.Done(); c, e := s.eventRepo.CountHealthKitByUser(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); healthKitCount = c; mu.Unlock() }()
	go func() { defer wg.Done(); c, e := s.changeLogRepo.GetLatestCursor(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); latestCursor = c; mu.Unlock() }()
	go func() { defer wg.Done(); t, e := s.eventRepo.GetLatestTimestamp(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); lastEvent = t; mu.Unlock() }()
	go func() { defer wg.Done(); t, e := s.eventTypeRepo.GetLatestTimestamp(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); lastEventType = t; mu.Unlock() }()
	go func() { defer wg.Done(); t, e := s.eventRepo.GetLatestHealthKitTimestamp(ctx, userID); if e != nil { setError(e); return }; mu.Lock(); lastHealthKit = t; mu.Unlock() }()
	wg.Wait()
...
func computeSyncRecommendations(eventCount, eventTypeCount, healthKitCount, latestCursor int64, lastEvent, lastEventType *time.Time) (string, []string) {
	var recs []string
	if eventTypeCount == 0 { recs = append(recs, "No event types found. Create event types to start tracking.") }
	if eventCount == 0 && eventTypeCount > 0 { recs = append(recs, "No events recorded yet. Start tracking to see data.") }
	status := "all_synced"
	...
	if len(recs) > 0 && status == "all_synced" { status = "pending_changes" }
	return status, recs
}
